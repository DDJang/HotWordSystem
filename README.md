...
| **并发处理** | 自定义 `ThreadPool`                                     | 动态线程池、任务调度、并行处理     |
| **内存管理** | `mimalloc`                                              | 高性能内存分配器                   |
| **日志**   | `spdlog`                                                | 快速日志库                         |
| **正则表达式** | `compile-time-regular-expressions`                     | 编译时正则表达式库                 |
...
2.  **高性能与并发安全**
    *   **高效的锁管理**: 系统采用全局锁和读写锁相结合的方式，通过**最小化锁持有时间**来降低锁竞争，在保证数据绝对一致性的同时，实现了高并发处理能力。
    ...
    *   **动态线程池**: 内置自定义 `ThreadPool` 实现，支持动态线程数调整、批量任务提交和异常处理，优化了并发性能。
    *   **高性能内存分配**: 集成 `mimalloc` 高性能内存分配器，减少内存碎片，提高内存分配效率。
...
4.  **健壮性设计**
    ...
    *   **异常处理**: 线程池和任务执行过程中包含完善的异常捕获和处理机制，确保系统稳定性。
    *   **缓存机制**: 文本处理结果缓存，减少重复计算，提高系统响应速度。
...
└── third_party/            # 第三方依赖库
    ├── cppjieba/           # 中文分词库
    ├── compile-time-regular-expressions/ # 编译时正则表达式库
    ├── mimalloc/           # 高性能内存分配器
    ├── spdlog/             # 快速日志库
    └── utfcpp/             # UTF-8 处理库
```
...
| `[ACTION] BENCHMARK_STOP`                                   | 停止正在进行的压力测试                                       |
...
*   **内存使用**: 默认配置下，为防止 OOM，系统限制最大物理存储条目为 200 万条。此上限可在 `SlidingWindow.h` 的 `MAX_STORAGE_ENTRIES` 常量中修改。
*   **时间戳处理**: 若输入的文本行**不包含** `[HH:MM:SS]` 格式的时间戳，系统会**自动将上次记录的时间戳加一秒**作为该行数据的时间戳。
*   **线程池**: 系统默认使用基于硬件并发数的动态线程池，可根据任务量自动调整线程数，提高并发处理能力。
*   **缓存机制**: 文本处理结果缓存默认大小为 10000 条，可根据系统内存情况进行调整。